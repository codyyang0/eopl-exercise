          ...
          (if (null? vars)
              (let ((val (value-of exp env)))
                (cases expval val
                  ...
                  (else
                    (report-error-binds))))
              (value-of (unpack-exp (cdr vars) (cdr-exp exp) body)
                        (extend-env (car vars) (value-of (car-exp exp) env) env))))   

        )))


  (define report-error-binds
    (lambda ()
      (eopl:error 'unpack-out-of-boundary
                  "unpack vars is not equal with vals")))
