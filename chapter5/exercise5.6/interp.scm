...
;;;;;;;;;;;;;;;; the interpreter ;;;;;;;;;;;;;;;;
  ...
  (define value-of/k
    (lambda (exp env cont)
      (cases expression exp
        ...
        ; Exercise 5.5
        (cons-exp (exp1 exp2)
          (value-of/k exp1 env (cons-exp1-cont exp2 env cont)))
        ...
        ; Exercise 5.6
        ; not easy understand, but correct
        (list-exp (exps)
          (apply-cont cont
            (apply-cont (list-rest-cont (cdr exps) env)
                        (value-of/k (car exps) env (list-first-cont)))))
  ...
  (define cons-exp1-cont
    (lambda (exp2 env cont)
      (lambda (val)
        (value-of/k exp2 env
                    (cons-exp2-cont val cont)))))

  ; cons-exp2-cont : ExpVal * Env * Cont -> Cont
  (define cons-exp2-cont
    (lambda (val1 cont)
      (lambda (val)
        (apply-cont cont (cons-val val1 val)))))
  ...
  (define null?-exp-cont
    (lambda (cont)
      (lambda (val)
        (cases expval val
          (emptylist-val () (apply-cont cont (bool-val #t)))
          (else (apply-cont cont (bool-val #f)))))))

  ; Exercise 5.6
  ; list-first-cont : Cont -> Cont
  (define list-first-cont
    (lambda ()
      (lambda (val) val)))

  ; list-rest-cont : Env * Cont -> Cont
    ...
    (lambda (exps env)
      (lambda (val)
        (if (null? exps)
            (emptylist-val)
            (cons-val val
                      (value-of/k (car exps) env (list-rest-cont (cdr exps) env)))))))
  )
