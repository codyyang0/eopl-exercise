...
;;;;;;;;;;;;;;;; the interpreter ;;;;;;;;;;;;;;;;
          ...
          (let [[num (expval->num (value-of exp1 env))]
            ...
            (letrec [[inner-list
                      (lambda (num result)
                        (if (zero? num)
                            result
                            (inner-list (- num 1) (cons val result))))]]
              (let [[vals (inner-list num '())]]
                (array-val (make-array vals))))))
  ...
  ;; if the expression is a array-ref-exp, then pass the reference.
