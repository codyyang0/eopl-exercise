...
"letrec int even(odd : (int -> int))  = proc(x : int) if zero?(x) then 1 else (odd -(x,1))
   ...
   in (odd 13)" 1)
      ...
      (unpair-typed
       "let x = 1 in let y = 2 in let z = newpair(x, y) in unpair a b = z in zero?(a)" #f)
...
in letrec 
   ...
   in (fact 4)"
      ...
      (build-a-pair-typed
       "let x = 1 in let y = 2 in newpair(x, y)" (int int))

      (unpair-typed
       "let x = 1 in let y = 2 in let z = newpair(x, y) in unpair a b = z in zero?(a)" bool)
